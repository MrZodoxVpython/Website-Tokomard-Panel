...
if (isset($_GET['hapus'])) {
    ...
    header("Location: show-shadowsocks-sgdo-2dev.php");
    exit;
}
    ...
    if (isset($_POST['edit_user'])) {
        ...
        if ($expiredBaru) {
            ...
            foreach ($lines as $i => $line) {
                if (preg_match('/^\s*#ss(grpc)?$/i', trim($line), $m)) {
                    $currentTag = '#' . strtolower($m[1] . ($m[2] ?? ''));
                }

                if (in_array($currentTag, ['#ss', '#ssgrpc'])) {
                    if (preg_match('/^\s*#$\s+' . preg_quote($userEdit, '/') . '\s+\d{4}-\d{2}-\d{2}/', $line, $matches)) {
                        $prefix = $matches[1];
                    ...
                    if (preg_match('/"password"\s*:\s*"locked"/', $jsonLine)) {
                        ...
                        if (preg_match('/^##LOCK##(.+)/', $lockLine, $m)) {
                            ...
                            if (strpos($jsonLine, '"email": "' . $user . '"') !== false) {
                                $lines[$jsonLineIndex] = preg_replace('/"password"\s*:\s*"locked"/', '"password": "' . $realId . '"', $jsonLine) . "\n";
...
<head>
    <meta charset="UTF-8">
    <title>Shadowsocks - SGDO-2DEV</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-900 text-white p-6">
<div class="max-w-4xl mx-auto">
    <h1 class="text-center text-2xl font-bold mb-4">Daftar Akun Shadowsocks (SGDO-2DEV) - <?= htmlspecialchars($reseller) ?></h1>
...
<?php foreach ($akunFiles as $file):
    ...
    if (stripos($content, '"protocol": "shadowsocks"') === false && stripos($content, 'ss://') === false) {
        continue; // Lewati Trojan, dll
    }
