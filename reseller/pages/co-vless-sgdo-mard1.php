...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$username || !$expiredInput) {
        ...
        if ($saldoUser < $hargaFinal) {
            ...
            $remoteIp = $server['ip'];
            $phpCmd = "php /etc/xray/api-akun/add-vless.php '$username' '$expiredInput' '$password' '$reseller'";
            ...
            if (!empty(trim($output)) && str_contains($output, 'VLESS ACCOUNT')) {
                ...
                if ($stmt->execute()) {
                    ...
                    $detail = 'Pembelian Vless SGDO-MARD1';
                    $type = 'buy';
                    $status = 'SUCCESS';
...
<head>
    <meta charset="UTF-8">
    <title>Checkout Vless SGDO-MARD1</title>
...
<div class="w-full max-w-2xl bg-white dark:bg-gray-900 shadow-md rounded-2xl p-6 space-y-6 border border-gray-200 dark:border-gray-700">
    ...
    <h3 class="text-xl font-semibold">🧾 Buat Akun Vless</h3>
