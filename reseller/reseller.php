  ...
  <aside id="sidebar" class="md:w-1/5 w-full md:max-w-xs bg-gray-100 dark:bg-gray-800 p-5 shadow-lg rounded-lg transition-transform duration-300 -translate-x-full md:translate-x-0 z-40 md:mr-1">
    ...
    <nav class="space-y-2 text-sm">
      ...
      foreach ($menu as $p => $label) {
          ...
          if ($p === 'shadowsocks') echo "<hr class='my-2 border-t dark:border-gray-600'>";
      }
      ?>
    </nav>
  </aside>
    ...
    if ($page === 'dashboard') {
        ...
        foreach (glob("$dir/akun-$reseller-*.txt") as $file) {
            ...
            foreach (['vmess','vless','trojan','shadowsocks'] as $proto) {
                if (stripos($content, strtoupper($proto) . " ACCOUNT") !== false) {
                    $stats[$proto]++;
                    $stats['total']++;
                    $rows[] = [
                        'no' => $no++,
                        'user' => strtoupper(substr($buyer, 0, 10)),
        ...
        // Grafik modern
        echo '
        <div class="bg-gray-100 dark:bg-gray-800 p-4 rounded-xl mb-8 shadow">
        ...
        // Tabel akun
        echo '<div class="overflow-x-auto"><table class="w-full table-fixed border border-gray-300 dark:border-gray-700 text-sm">
        <thead class="bg-gray-200 dark:bg-gray-700">
        <tr>
            <th class="p-2 w-10">#</th>
            <th class="p-2 w-1/4">User</th>
            <th class="p-2 w-1/6">Proto</th>
            <th class="p-2 w-1/4">Expired</th>
            <th class="p-2 w-1/4">Buyer</th>
        </tr>
        </thead>
        <tbody>';
        if (empty($rows)) {
            ...
            foreach ($rows as $r) {
                echo "<tr class='hover:bg-gray-100 dark:hover:bg-gray-700'>
                    ...
                    <td class='p-2'>{$r['proto']}</td>
                    <td class='p-2'>{$r['exp']}</td>
                    <td class='p-2'>{$r['buyer']}</td>
                </tr>";
            }
        }
