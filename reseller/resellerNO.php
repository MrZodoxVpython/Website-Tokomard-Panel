        ...
        <?php foreach ($notifications as $n): ?>
          <div class="flex items-start gap-3 px-5 py-4 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-150 <?= $n['sudah_dibaca'] ? '' : 'bg-blue-50 dark:bg-blue-900' ?>">
            <div class="flex-1">
              ...
              <span class="text-xs text-gray-500 dark:text-gray-400 block mt-1"><?= date('d M H:i', strtotime($n['dibuat_pada'])) ?></span>
            </div>
  ...
  <section class="flex-1 p-5 bg-white dark:bg-gray-900 rounded-xl shadow-md">
    <?php if ($page === 'dashboard'): ?>
      ...
      <div class="text-center grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
        <?php foreach (['total' => 'Total Akun', 'vmess' => 'VMess', 'vless' => 'VLess', 'trojan' => 'Trojan', 'shadowsocks' => 'Shadowsocks'] as $k => $label): ?>
          <?php $color = ['total' => 'green', 'vmess' => 'blue', 'vless' => 'purple', 'trojan' => 'red', 'shadowsocks' => 'yellow'][$k]; ?>
          <div class="bg-<?= $color ?>-100 dark:bg-<?= $color ?>-800 text-<?= $color ?>-900 dark:text-white p-5 rounded-lg shadow">
    ...
    <?php elseif (file_exists($pagePath)): ?>
      <?php include $pagePath; ?>
    <?php else: ?>
      <div class="text-red-500 text-center">Halaman <b><?= $page ?></b> tidak ditemukan.</div>
    <?php endif; ?>
  </section>
</main>
