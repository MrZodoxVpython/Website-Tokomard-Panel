...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$username || !$expiredInput) {
        ...
        if ($saldoUser < $hargaFinal) {
            ...
            // Panggil file lokal add-shadowsocks.php (bukan SSH)
            $phpCmd = "php /var/www/html/Website-Tokomard-Panel/reseller/pages/api-akun/add-shadowsocks.php '$username' '$expiredInput' '$password' '$reseller'";
            $outputRaw = shell_exec($phpCmd . ' 2>&1');

            if (!empty(trim($outputRaw)) && str_contains($outputRaw, 'SHADOWSOCKS ACCOUNT')) {
                // Kurangi saldo
                ...
                if ($stmt->execute()) {
                    ...
                    $detail = 'Pembelian Shadowsocks SGDO-2DEV';
                    $type = 'buy';
                    $status = 'SUCCESS';
...
<head>
    <meta charset="UTF-8">
    <title>Checkout Shadowsocks SGDO-2DEV</title>
...
<div class="w-full max-w-2xl bg-white dark:bg-gray-900 shadow-md rounded-2xl p-6 space-y-6 border border-gray-200 dark:border-gray-700">
    ...
    <h3 class="text-xl font-semibold">🧾 Buat Akun Shadowsocks</h3>
