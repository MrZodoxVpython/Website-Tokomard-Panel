...
.spin-hover:not(:hover) {
  animation: spin-reverse-once 0.5s linear 1;
}
</style>
<body class="bg-white dark:bg-gray-900 text-gray-900 dark:text-white">

<!-- HEADER -->
<header class="flex items-center justify-between bg-gray-100 dark:bg-gray-800 p-2.5 shadow sticky top-0 z-20">
  ...
  <div class="flex items-center space-x-3">
    ...
    <div class="relative cursor-pointer" onclick="toggleNotif(event)">
      ...
      <?php if ($notifCount): ?>
      <span id="notificationIndicator" class="absolute top-0 right-0 bg-red-600 text-white text-xs rounded-full px-1 animate-pulse"><?= $notifCount>9?'9+':$notifCount ?></span>
      <?php endif; ?>
    </div>
...
<!-- SIDEBAR -->
<div class="flex">
<aside id="sidebar" class="pb-10 fixed inset-y-0 left-0 bg-white dark:bg-gray-800 w-64 p-4 transform -translate-x-full md:translate-x-0 transition-transform z-20">
  <!-- Avatar Upload -->
