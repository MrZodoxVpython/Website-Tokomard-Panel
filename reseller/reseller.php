...
<aside id="sidebar" class="fixed inset-y-0 left-0 bg-white dark:bg-gray-800 w-64 p-4 transform -translate-x-full md:translate-x-0 transition-transform z-20">
  ...
  <div class="relative text-center font-semibold cursor-pointer -mt-2 mb-8" onclick="toggleNotif()">
    ...
    <?php if ($notifCount > 0): ?>
    <span class="absolute -top-0.5 right-7 w-4 h-8 bg-red-600 text-white text-[10px] flex items-center justify-center font-bold rounded-full animate-pulse z-10">
      <?= $notifCount > 9 ? '9+' : $notifCount ?>
    </span>
    <?php endif; ?>
  </div>
