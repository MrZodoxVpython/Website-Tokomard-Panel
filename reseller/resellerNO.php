...
?>
<!DOCTYPE html>
<html class="<?= isset($theme) && $theme === 'dark' ? 'dark' : '' ?>">
<head>
  <meta charset="UTF-8">
...
<!-- HEADER -->
<header class="px-4 py-3 bg-gray-100 dark:bg-gray-800 shadow-md flex justify-between items-center sticky top-0 z-20">
  <div class="flex items-center gap-3 cursor-pointer relative" onclick="document.getElementById('notifDropdown').classList.toggle('hidden')">
    <div class="relative">
      ...
      <span class="absolute top-1 right-1 transform translate-x-1/4 -translate-y-1/4 w-5 h-5 text-[12px] flex items-center justify-center font-bold text-white bg-red-600 border-2 border-white dark:border-gray-800 rounded-full animate-pulse">
  ...
  <div class="flex items-center gap-4">
    <button onclick="toggleTheme()" class="text-xl p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700">
      <span id="themeIcon"><?= isset($theme) && $theme === 'dark' ? '🌞' : '🌙' ?></span>
    </button>
...
<!-- DROPDOWN NOTIFIKASI -->
<div class="relative">
  <div id="notifDropdown" class="hidden absolute top-16 right-4 w-full max-w-md bg-white dark:bg-gray-900 shadow-xl rounded-xl border border-gray-200 dark:border-gray-700 z-50">
    ...
    <?php if (!empty($notifications)): ?>
      <div class="divide-y divide-gray-200 dark:divide-gray-700 max-h-96 overflow-y-auto">
        <?php foreach ($notifications as $n): ?>
          <div class="flex items-start gap-3 px-5 py-4 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-150 <?= !empty($n['sudah_dibaca']) ? '' : 'bg-blue-50 dark:bg-blue-900' ?>">
            <div class="flex-1">
              <p class="text-sm text-gray-800 dark:text-gray-200"><?= htmlspecialchars($n['pesan'] ?? '-') ?></p>
              <span class="text-xs text-gray-500 dark:text-gray-400 block mt-1"><?= date('d M H:i', strtotime($n['dibuat_pada'] ?? 'now')) ?></span>
            </div>
            <?php if (empty($n['sudah_dibaca'])): ?>
              <span class="inline-block mt-1 bg-blue-600 text-white text-[10px] font-semibold px-2 py-0.5 rounded-full">Baru</span>
            <?php endif; ?>
          </div>
        <?php endforeach; ?>
      </div>
    <?php else: ?>
      <div class="px-5 py-6 text-center text-sm text-gray-500 dark:text-gray-400 italic">Tidak ada notifikasi baru.</div>
    <?php endif; ?>
...
<main class="flex flex-col md:flex-row w-full px-4 md:px-8 py-6 gap-6">
  <!-- SIDEBAR -->
  <aside id="sidebar" class="md:w-1/5 w-full md:max-w-xs bg-gray-100 dark:bg-gray-800 p-5 shadow-lg rounded-lg transition-transform duration-300 fixed md:static top-0 left-0 h-full z-40 md:translate-x-0 -translate-x-full">
    <div class="flex flex-col items-center text-center mb-6">
      <form action="upload-avatar.php" method="POST" enctype="multipart/form-data">
        <label for="avatarUpload" class="cursor-pointer">
          <img src="<?= isset($avatarPath) ? $avatarPath . '?v=' . time() : '#' ?>" class="w-20 h-20 rounded-full mb-2 hover:opacity-80 transition" />
        </label>
    ...
    <nav class="space-y-2 text-sm">
      <?php if (!empty($menus)): foreach ($menus as $key => $label): ?>
        <a href="?page=<?= $key ?>" class="block px-3 py-2 rounded hover:bg-blue-500 hover:text-white dark:hover:bg-blue-600"><?= $label ?></a>
        <?php if ($key === 'shadowsocks') echo "<hr class='border-t border-gray-400 dark:border-gray-600 my-2' />"; ?>
  ...
  <section class="flex-1 p-5 bg-white dark:bg-gray-900 rounded-xl shadow-md">
    <?php if (($page ?? '') === 'dashboard'): ?>
      <!-- Statistik -->
      <div class="text-center grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
        <?php
        $labels = ['total' => 'Total Akun', 'vmess' => 'VMess', 'vless' => 'VLess', 'trojan' => 'Trojan', 'shadowsocks' => 'Shadowsocks'];
        $colors = ['total' => 'green', 'vmess' => 'blue', 'vless' => 'purple', 'trojan' => 'red', 'shadowsocks' => 'yellow'];
        foreach ($labels as $k => $label):
        ?>
          <div class="bg-<?= $colors[$k] ?>-100 dark:bg-<?= $colors[$k] ?>-800 text-<?= $colors[$k] ?>-900 dark:text-white p-5 rounded-lg shadow">
            <p class="text-lg font-semibold"><?= $label ?></p>
            <p class="text-3xl mt-2 font-bold"><?= $stats[$k] ?? 0 ?></p>
          </div>
        <?php endforeach; ?>
      </div>

      <!-- Chart -->
      <div class="mb-8 max-w-full bg-white dark:bg-gray-800 p-4 rounded-lg shadow overflow-auto">
        <canvas id="myChart" class="h-[450px] w-full"></canvas>
      </div>

      <!-- Tabel -->
      <div class="overflow-x-auto">
        <table class="table-auto min-w-full border border-gray-300 dark:border-gray-700 text-sm text-left">
          ...
          <tbody>
            <?php if (empty($rows)): ?>
              <tr><td colspan="5" class="text-center px-3 py-4 text-gray-500 dark:text-gray-400">Belum ada akun.</td></tr>
    ...
    <?php elseif (!empty($pagePath) && file_exists($pagePath)): ?>
