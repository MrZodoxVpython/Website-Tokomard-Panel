...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$username || !$expiredInput) {
        ...
        if ($saldoUser < $hargaFinal) {
            ...
            $phpCmd = "php /var/www/html/Website-Tokomard-Panel/reseller/pages/api-akun/add-ssh.php '$username' '$expiredInput' '$password' '$reseller'";
            $outputRaw = shell_exec($phpCmd . ' 2>&1');

            if (!empty(trim($outputRaw)) && str_contains($outputRaw, 'SSH ACCOUNT')) {
                // Kurangi saldo
                ...
                if ($stmt->execute()) {
                    ...
                    $detail = 'Pembelian SSH SGDO-2DEV';
                    $type = 'buy';
                    $status = 'SUCCESS';
...
<head>
    <meta charset="UTF-8">
    <title>Checkout SSH SGDO-2DEV</title>
