...
if (isset($_GET['hapus'])) {
    ...
    for ($i = 0; $i < count($lines); $i++) {
        ...
        if (preg_match('/^##LOCK##/', trim($lines[$i]))) continue;
        $newLines[] = $lines[$i];
    }

    file_put_contents($configPath, implode('', $newLines));
    shell_exec('sudo /usr/local/bin/restart-xray.sh');
    foreach (glob("$logDir/akun-$reseller-$userToDelete.txt") as $file) unlink($file);
        ...
        <div class="bg-gray-800 rounded shadow mb-6 p-4">
            <div class="flex justify-between items-center flex-wrap">
                ...
                <div class="flex gap-2 mt-2 sm:mt-0 flex-wrap">
                    ...
                    <form method="GET" action="" class="inline" onsubmit="return confirm('Yakin ingin menghapus akun ini?')">
