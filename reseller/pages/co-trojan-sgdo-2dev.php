...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$username || !$expiredInput) {
        ...
        if ($saldoUser < $hargaFinal) {
            ...
            $phpCmd = "php /var/www/html/Website-Tokomard-Panel/reseller/pages/api-akun/add-trojan.php '$username' '$expiredInput' '$password' '$reseller'";
            $outputRaw = shell_exec($phpCmd . ' 2>&1');

            if (!empty(trim($outputRaw)) && str_contains($outputRaw, 'TROJAN ACCOUNT')) {
                // Kurangi saldo
...
<head>
    <meta charset="UTF-8">
    <title>Checkout Trojan SGDO-2DEV</title>
