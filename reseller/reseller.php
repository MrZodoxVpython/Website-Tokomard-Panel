<?php
session_start();
require 'koneksi.php'; // sambungan ke DB
...
// Ambil notifikasi
$stmt = $conn->prepare("
  SELECT id, pesan, sudah_dibaca, dibuat_pada 
...
<head>
  ...
  <title><?= $notifCount ? "($notifCount) " : '' ?>Tokomard Panel</title>
...
<header class="px-3 py-2 bg-gray-100 dark:bg-gray-800 shadow-md flex justify-between items-center sticky top-0 z-10">
  <div class="flex items-center space-x-3">
    <img src="https://i.imgur.com/q3DzxiB.png" class="w-10 cursor-pointer" alt="Logo"
...
<div id="notifDropdown" class="hidden absolute top-16 left-4 w-80 bg-gray-50 dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden z-20">
  ...
  <?php foreach ($notifications as $n): ?>
    <div class="px-4 py-2 flex justify-between items-center <?= $n['sudah_dibaca'] ? 'bg-gray-100 dark:bg-gray-700' : 'bg-blue-100 dark:bg-blue-700' ?>">
      ...
      <span class="text-xs text-gray-500"><?= date('d M H:i', strtotime($n['dibuat_pada'] ?? $n['created_at'])) ?></span>
    </div>
  <?php endforeach ?>
...
<main class="flex flex-col md:flex-row w-full px-4 md:px-8 py-6 gap-6">
  <aside id="sidebar" class="md:w-1/5 bg-gray-100 dark:bg-gray-800 p-5 shadow-lg rounded-lg">
    <div class="flex flex-col items-center text-center mb-6">
      <form id="avatarForm" action="upload-avatar.php" method="POST" enctype="multipart/form-data">
        <label for="avatarUpload" class="cursor-pointer">
          <img src="<?= $avatar ?>?v=<?= time() ?>" class="w-20 h-20 rounded-full mb-2 hover:opacity-80 transition" />
        </label>
    ...
    <nav class="space-y-2 text-sm">
      <?php
      $menus = ['dashboard'=>'📊 Dashboard','ssh'=>'🔐 SSH','vmess'=>'🌀 Vmess','vless'=>'📡 Vless',
  ...
  <section class="flex-1 p-5 bg-white dark:bg-gray-900 rounded-xl shadow-md">
    <?php
    // konten halaman sesuai page
    $pagePath = __DIR__ . "/pages/{$page}.php";
    if ($page === 'dashboard') { /*...kode statistik seperti sebelumnya...*/ }
    elseif (file_exists($pagePath)) include $pagePath;
    else echo "<div class='text-red-500 text-center'>Halaman <b>{$page}</b> tidak ditemukan.</div>";
    ?>
  </section>
</main>
